...
--
- $Y_k$ is a good estimate of $Y_i(0)$
...
$$\overbrace{\text{argmax}_{\boldsymbol{\theta} \in \{0, 1\}^p}\;\boldsymbol{\theta}^T\mathbf{w}}^{\text{most important covariate set}}\quad\text{s.t.}\\\quad \color{blue}{\exists k\;\:\text{with}\;\: \underbrace{\mathbf{x}_{k} \circ \boldsymbol{\theta} = \mathbf{x}_{i} \circ \boldsymbol{\theta}}_{\text{exact matching on }\boldsymbol{\theta}} \;\:\text{and}\;\: \underbrace{T_{k} = 1 -T_i}_{\text{opposite treatment}}}$$
...
---
# Missing Data
`missing_data`: how missing values in `data` to be matched are treated
