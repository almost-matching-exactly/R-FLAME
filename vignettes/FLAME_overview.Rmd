...
1. There is an extra logical column, `FLAME_out$data$matched`, that indicates whether or not a unit was matched. This can be useful if, for example, you'd like to use only the matched data for subsequent analysis:
```{r}
matched_data <- dplyr::filter(FLAME_out$data, matched)
```

2. There is an extra numeric column, `FLAME_out$data$weight` that denotes on how many different sets of covariates a unit was matched. By default, this will be 1 if a unit is matched and 0 otherwise. With the `repeats = TRUE` argument, however, units are allowed to match several times on multiple sets of covariates and their values for `weight` can therefore be greater than 1.  These weights can be used to estimate average treatment effects.
...
* `treated_column_name`: A character with the name of the treatment column in `data`. Defaults to 'treated'. 
...
* `early_stop_epsilon`: If the predictive error at any iteration rises above 100 * early_stop_epsilon percent of the error associated with predicting the outcome using _all_ covariates, FLAME exits. 
...
`CATE(units, FLAME_out)` takes in `units`, the units for which you would like an estimate of the conditional treatment effect, and the output of a run of FLAME, `FLAME_out`. You may supply only a single unit for the `units` argument. 

In this way, we can find the treatment effect for a unit of interest, say, unit 5...
```{r,eval=FALSE}
CATE(units = 5, FLAME_out = FLAME_out)
```

... or for a class of units for which the treatment effect is of particular interest:
```{r,eval=FALSE}
CATE(units = which(data$age < 30 & data$treated == 1), FLAME_out = FLAME_out)
```

`ATE(FLAME_out)` and `ATT(FLAME_out)` return the estimated average treatment effect and the estimated average treatment effect on the treated, respectively. 
```{r,eval=FALSE}
ATE(FLAME_out = FLAME_out)
ATT(FLAME_out = FLAME_out)
```

Lastly, we can use `MG(units, FLAME_out)` to get a list of the members of the matched groups of all units in `units`:
```{r, eval = FALSE}
MG(units = 5, FLAME_out = FLAME_out)
MG(units = which(data$age < 30 & data$treated == 1), FLAME_out = FLAME_out)
```

Note that if the value supplied to the `holdout` argument is a proportion between 0 and 1, the values supplied to the `units` argument in the functions above will be indexed with respect to `FLAME_out$data` _and not with respect to the data passed in _`data`.
